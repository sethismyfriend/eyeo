...
int32_t galileoID = 2;         //set unique for 0-3 for each touchpoint
...
//This method goes with the darlington circuit in the docs folder. 
void readBetweenColumns() {
  //COUNTING BELOW AND ABOVE THRESHOLDS
  ...
  //AVERAGING METHOD
  if((sensorAvg < lowThreshold) && (holdingHands == 0)) {
      holdingHands = 1; 
      if(useSerial) Serial.print("HOLDING HANDS!"); 
      sendResistiveChange(0, holdingHands, sensorBuffer);
  }
  if((sensorAvg > highThreshold) && (holdingHands == 1)) {
      holdingHands = 0; 
      if(useSerial) Serial.print("RELEASE HANDS!");
      sendResistiveChange(0, holdingHands, sensorBuffer);
  }
}
...
void readBetweenColumnsRESISTIVE() {
  //column 1
  sensorBuffer = analogRead(SENSOR_PIN);  // get new sensor value to examine
  sensorAvg = (sensorAvg * numToAvg + sensorBuffer)/(numToAvg + 1); //caclulates a running average to filter noise
